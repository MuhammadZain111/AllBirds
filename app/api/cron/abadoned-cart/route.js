...
//
// This route is called by an external cron scheduler every hour (or 30 min).
// Recommended schedulers:
//   • Vercel Cron Jobs  (vercel.json → "crons" array)
//   • GitHub Actions    (schedule: "0 * * * *")
//   • cron-job.org      (free, external HTTP ping)
//
  ...
  try {
    ...
    for (const cart of abandonedCarts) {
      try {
        ...
        successCount++;
        console.log(`[AbandonedCart] Email sent to ${cart.userEmail} (cartId: ${cart._id})`);
      } catch (emailError) {
        failCount++;
        errors.push({ cartId: cart._id, error: emailError.message });
        console.error(`[AbandonedCart] Failed for ${cart.userEmail}:`, emailError.message);
      }
    }
